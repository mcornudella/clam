    		...
    		<div id="tabs">
    			<ol>
                    ...
                    <xsl:when test="/clam/@oauth_access_token = ''">
                      <li class ="restart"><a href="{/clam/@baseurl}/" style="text-decoration: none"> Restart</a></li>
                    </xsl:when>
                    <xsl:otherwise>
                      <li class="restart"><a href="{/clam/@baseurl}/?oauth_access_token={/clam/@oauth_access_token}" style="text-decoration: none"> Restart</a></li>
                    </xsl:otherwise>
                    </xsl:choose>
				    <xsl:choose>
          ...
          <xsl:if test="/clam/@oauth_access_token != ''">
            ...
            <xsl:choose>
              <xsl:when test="status/@code = 0">
                ...
                <div class="box" style="border: none; margin-top: 10px;">
    			   <div id="startbutton">
...
<xsl:template match="/clam/profiles">
	<h2> 2.- Choose how to upload files: </h2>
        <div id="uploadarea">
            ...
            <div class="uploadform">
                ...
                <!--  <p style="padding: 7px;">Use this to upload files from your computer to the system. </p> -->
                    ...
                    <!--  <xsl:text> </xsl:text><p style="padding: 7px;">Click the upload button below and then select one or more files (holding control), you can also drag &amp; drop files onto the button from an external file manager</p><xsl:text> </xsl:text> -->
            ...
            <xsl:if test="contains(/clam/@interfaceoptions,'inputfromweb')">
				<div class="urluploadform">
		            ...
		            <div id="urlupload" style="display: none;">
		                ...
		                <xsl:text> </xsl:text> <p style="padding: 7px;"><input id="urluploadsubmit" class="uploadbutton" type="submit" value="Retrieve and add file" /></p>
        ...
        <table id="inputfiles" class="files">
            <thead>
                <tr>
                    ...
                    <th style="width: 20%; background-color: white;">Template</th>
...
<xsl:template match="/clam/output/file">
 <!--  <xsl:when test="./name != error.log"> -->
    <tr>
    	 <xsl:if test="not(./name = 'error.log')">
	        <td class="file">
	        <xsl:choose>
	        ...
	        <td>
	            ...
	            <a><xsl:attribute name="href"><xsl:value-of select="@xlink:href" /> <xsl:if test="/clam/@oauth_access_token != ''">?oauth_access_token=<xsl:value-of select="/clam/@oauth_access_token"/></xsl:if> </xsl:attribute><xsl:attribute name="download"></xsl:attribute> Download</a>
